...
export class ReconstructCommand extends Command {
    ...
    async execute(args, options, context) {
        try {
            ...
            // Load snapshot
            const snapshot = await loadSnapshot(filePath ? await this.readFile(filePath) : context.stdin, options);
            ...
            return this.error(`Reconstruct failed: ${err.message}`, options);
        }
    }
    parseArgs(args) {
        const flags = {};
    ...
    async compareWithSnapshot(snapshot, directory, _flags) {
        ...
        const matches = missingFiles.length === 0 &&
            extraFiles.length === 0 &&
    ...
    collectLocalFiles(baseDir, currentDir, fileSet) {
        try {
    ...
    outputResults(result, options, flags) {
        if (options.json) {
