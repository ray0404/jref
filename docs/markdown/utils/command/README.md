[**jref - JSON Reference Tool v1.4.0**](../../README.md)

***

[jref - JSON Reference Tool](../../modules.md) / utils/command

# utils/command

## Classes

- [Command](classes/Command.md)
- [CommandRegistry](classes/CommandRegistry.md)

## Interfaces

- [CommandOption](interfaces/CommandOption.md)
- [CommandDefinition](interfaces/CommandDefinition.md)
- [JrefPlugin](interfaces/JrefPlugin.md)

## Variables

- [registry](variables/registry.md)

## Functions

- [loadPlugins](functions/loadPlugins.md)
- [registerBuiltinCommands](functions/registerBuiltinCommands.md)
